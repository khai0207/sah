...
class TwilightManager {
    ...
    private void updateState(Location location) {
        ...
        twilightCalculator.calculateTwilight(currentTimeMillis - a.h, location.getLatitude(), location.getLongitude());
        ...
        boolean z2 = z;
        twilightCalculator.calculateTwilight(a.h + currentTimeMillis, location.getLatitude(), location.getLongitude());
        long j5 = twilightCalculator.sunrise;
        if (j3 == -1 || j4 == -1) {
            j = 43200000 + currentTimeMillis;
        } else {
            j = (currentTimeMillis > j4 ? 0 + j5 : currentTimeMillis > j3 ? 0 + j4 : 0 + j3) + KV.GET_CODE_INTERVAL;
        }
