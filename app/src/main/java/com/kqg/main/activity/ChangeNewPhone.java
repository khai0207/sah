...
public class ChangeNewPhone extends BaseActivity {
    ...
    protected void afterOnCreate() {
        ...
        this.old_phone1 = getIntent().getBundleExtra("bundle").getString("old_phone");
        this.user = (User) getIntent().getSerializableExtra("user");
        setView("kqg_change_phone");
        this.pwd_close = (Button) getView("pwd_close");
        this.sure_bt = (Button) getView("sure_bt");
        ((TextView) getView("desc_tx")).setText(UiUtils.getResString("change_phone_5"));
        ...
        this.edit_old_phone.setHint(new SpannableString(UiUtils.getResString("change_phone_3")));
        ...
        this.verificationSeekBar.setOnEndListener(new VerificationSeekBar.onEndListener() { // from class: com.kqg.main.activity.ChangeNewPhone.1
            AnonymousClass1() {
            }

            @Override // com.kqg.main.view.VerificationSeekBar.onEndListener
            public void onEnd(String str) {
                ChangeNewPhone.this.verificationSeekBar.setVisible(4);
                ChangeNewPhone.this.verificationSeekBar.setProgress(0);
                if (str == "btn_get_phone_code" && str == "btn_get_phone_code") {
                    JSONObject jSONObject = new JSONObject();
                    try {
                        jSONObject.put("phone", ChangeNewPhone.this.edit_old_phone.getText().toString().trim());
                        jSONObject.put("type", aj.a);
                        jSONObject.put(SpeechEvent.KEY_EVENT_SESSION_ID, ChangeNewPhone.this.user.getSession_id());
                    ...
                    ChangeNewPhone.this.postMessageOnBackgroundThread(backGroundMessage);
                }
            }
            ...
            public void resetView(String str) {
                if (str == "btn_get_phone_code") {
                    ChangeNewPhone.this.btn_get_phone_code.setVisibility(0);
                }
            }
        });
        long currentTimeMillis = System.currentTimeMillis() - CountDownTimerRecord.change_new_phone_code_time;
    ...
    /* renamed from: com.kqg.main.activity.ChangeNewPhone$1 */
    /* loaded from: classes.dex */
    ...
    public void click(View view) {
        ...
        if (id == UiUtils.getId("sure_bt")) {
            ...
            if (checkIsSamePhone()) {
                UiUtils.showImageToast(UiUtils.getResString("warn_phone_equal"));
                return;
            }
            ...
            try {
                ...
                jSONObject.put(SpeechEvent.KEY_EVENT_SESSION_ID, this.session_id);
                jSONObject.put(Constants.URL_ENCODING, KV.CHANGE_NEW_PHONE_CHECK_CODE);
                jSONObject.put("isPost", "0");
                jSONObject.put("what", KV.EVENT_CHECK_CODE2);
                CommonEntity commonEntity = new CommonEntity();
                commonEntity.setJson(jSONObject);
    ...
    public void handleMessage(Message message) {
        ...
        int i = message.what;
        if (i == 1020) {
            Log.v("kqgsdk", "修改新的手机--验证码获取成功");
            UiUtils.showImageToast(UiUtils.getResString("code_send_success"));
            CountDownTimerRecord.change_new_phone_code_time = System.currentTimeMillis();
        ...
        Log.v("kqgsdk", "修改新的手机--完成");
        UiUtils.showImageToast(UiUtils.getResString("change_phone_success"));
        UserManager.getInstance().removeUser(this.user);
        finishAllActivities();
        startActivity(new Intent(this, (Class<?>) LoginActivity.class));
    }

    private boolean checkIsSamePhone() {
        return this.old_phone1.trim().equals(this.edit_old_phone.getText().toString().trim());
    }
