    ...
    --if self.m_enemy_object_table then
        table.foreach(self.m_enemy_object_table, function(i, v)
            ...
            if hpValue == nil or hpValue == 0 or ((self.m_skill_type=="skill_hero") and (self.m_anchor_index>0)) or ((self.m_skill_type=="skill_pet") and (self.m_anchor_index>0)) or ((self.m_skill_type=="skill_combiner") and (self.m_anchor_index>0))  then 
                ...
                local currentHp = v.new_tempCur_HP
                local maxHp = v.new_tempMAX_HP
                if(currentHp<=0)then
                ...
                tempHp = tempHp < 0 and 0 or tempHp
                tempHp = tempHp > maxHp and maxHp or tempHp
                -- cclog("skill hpValue ======================" .. currentHp .. "-" .. hpValue .. "=" .. tempHp .. " ; id == " .. v.animNode:getId());
                v.new_tempCur_HP = tempHp
                v.bloodBar:setCurValue((tempHp*1000/maxHp),false);
...
function game_battle_skill.addHp(self)
    ...
    for posIndex,addHpValue in pairs(self.m_tableAddHp) do
        ...
        if add_hp_object then
            ...
            local currentHp = add_hp_object.new_tempCur_HP
            local max = add_hp_object.new_tempMAX_HP
            local tempHp = currentHp + tonumber(addHpValue)
            tempHp = tempHp > max and max or tempHp
            add_hp_object.new_tempCur_HP = tempHp
            add_hp_object.bloodBar:setCurValue((tempHp*1000/max),false);
