...
function game_battle_skill_pets.destroy(self)
    self.params = nil;
...
function game_battle_skill_pets.retain(self,fromStr)
...
function game_battle_skill_pets.release(self,fromStr)
    if self.m_uReference == nil then return end
    self.m_uReference = self.m_uReference - 1;
...
function game_battle_skill_pets.skillOvertest(self)
end
--[[--
    技能播放完
]]
function game_battle_skill_pets.skillOver(self)
    ...
    if (self.m_skill_type == "skill_pet") then
      cclog("skill end ============== skill is pet skill " .. tostring(self.m_anchor_num) .. " index:" .. tostring(self.m_anchor_index));
        self.params.game_battle_table:usePetSkill(self.m_skillid,self.m_camp)
    end
    if (self.m_skill_type == "skill_combiner") then
      cclog("skill end ============== skill is combiner skill " .. tostring(self.m_anchor_num) .. " index:" .. tostring(self.m_anchor_index));
        self.params.game_battle_table:useCombinerSkill(self.m_skillid,self.m_camp)
    end
    ...
    --if self.m_enemy_object_table then
        table.foreach(self.m_enemy_object_table, function(i, v)
            ...
            if hpValue == nil or hpValue == 0 or ((self.m_skill_type=="skill_hero") and (self.m_anchor_index>0)) or ((self.m_skill_type=="skill_pet") and (self.m_anchor_index>0)) or ((self.m_skill_type=="skill_combiner") and (self.m_anchor_index>0))  then 
    ...
    if self.m_skill_type=="skill_pet" then
        if self.m_attack_object.animNode then
            self.m_attack_object.animNode:removeFromParentAndCleanup(true)
        end
        self.m_attack_object = nil
    end
    if self.m_skill_type=="skill_combiner" then --召唤神兽？？？？
        if self.m_attack_object.animNode then
            self.m_attack_object.animNode:removeFromParentAndCleanup(true)
        end
        self.m_attack_object = nil
    ...
    if self.m_attack_object then
        ...
        local sc = animNode:getScale()
        local tempActScale
        if battleType == "zodiac_boss" then
          tempActScale = CCScaleTo:create(0.2,sc,sc);
        else
          tempActScale = CCScaleTo:create(0.2,1 * public_config.anim_scale,1 * public_config.anim_scale);
        end
...
function game_battle_skill_pets.addHp(self)
...
function game_battle_skill_pets.moveAnimNodeToCenter(self,t_params)
...
--复位
function game_battle_skill_pets.animAttackerPositionReset(self)
...
function game_battle_skill_pets.moveAnimNode(self,t_params)
...
function game_battle_skill_pets.moveToAnimNode(self,t_params)
...
function game_battle_skill_pets.scaleToAnimNode(self,t_params)
...
function game_battle_skill_pets.FadeAnimNode(self,t_params)
...
function game_battle_skill_pets.setFlipX(self,isFlipX)
    cclog("function:------------------------setFlipX");
   self.m_attack_object.animNode:setFlipX(isFlipX)
end

function game_battle_skill_pets.moveByAnimNode(self,t_params)
    ...
    if animCount > 0 then
       local function changeAni(animNode)
          ...
          if(animIndex <= animCount) then
             ...
             animNode:setRhythm(anim_sequence[animIndex][2]*public_config.action_rythm);
             game_util:playSection(animNode , anim_sequence[animIndex][1]);
             if(anim_sequence[animIndex][5] == 0) then
                local aniCircelPlay = function()
                   changeAni(animNode);
                end
    ...
    if animCount > 0 then
       local function changeAni(animNode)
          ...
          if(animIndex <= animCount) then
             ...
             game_util:playSection(animNode , anim_sequence[animCount][1]);
          end
       end
       local function animListenter(animNode,theid,labelname)
          if(anim_sequence[animIndex][5] == 0) then
             -- animNode:playSection(anim_sequence[animIndex][1]);
             animNode:setRhythm(anim_sequence[animIndex][2]*public_config.action_rythm);
             game_util:playSection(anim_sequence[animIndex][1]);
          else
             changeAni(animNode)
          end
       end
...
--宠物技能
local function runPetSkillOtherSequence(self,t_params,node,mother_sequence)
    --处理动画序列
    cclog("function:------------------------runHeroSkillOtherSequence");
    local other_sequence  = mother_sequence;
    local otherCount  = table.getn(other_sequence);
...
--神兽技能
local function runombinerSkillOtherSequence(self,t_params,node,mother_sequence)
    --处理动画序列
    cclog("function:------------------------runCombinerSkillOtherSequence");
    local other_sequence  = mother_sequence;
    local otherCount  = table.getn(other_sequence);
    local enemy_object_tab = t_params.enemy_object_tab;
    local info = self;
    ...
    for i = 1,otherCount do
       ...
       if mAnimNode then
           ...
           local function realPlayAni()
              local function anchorCallFunc(animNode , mId , strValue)
                cclog("--------------- Combiner skill anchor ----------- " .. strValue);
                self.m_anchor_index = self.params.game_battle_table:anchorAllCall(strValue,self.m_enemy_object_table,self.m_hurtValueTable,self.m_attack_effect,self.m_anchor_num,self.m_anchor_index,self.m_add_object_table,self.m_addValueTable,true,self.params.currentFrameData);
              end
              mAnimNode:registerScriptAnchor(anchorCallFunc);
              mAnimNode:setVisible(true)
              ...
              local endcallback = function(mAnimNode)
                 ...
                 -- if(not iscango) then
                    overCount = overCount - 1
                    if(overCount == 0) then
                        cclog("runCombinerSkillOtherSequenceskillOver")
                        self:skillOver()
                    end
                 -- end
                 --self:release("runOtherSequence");
...
-- 攻击者的攻击动作
function game_battle_skill_pets.createAttackerAnim(self,t_params)
    ...
    if t_params.attackPos then
        cclog("game_battle_skill_pets.createAttackerAnim->t_params.attackPos")
        ...
        if animCount > 0 then 
            ...
            local function animListenter( animNode , theid , labelname )    -- 中动画结束响应事件
                ...
                if animIndex > animCount then
                    ...
                    game_util:playSection(animNode , anim_label_name_cfg.daiji);
                    --[[--if enemy_object_tab then
                    ...
                    if labelname == anim_label_name_cfg.qianjin then
                        ...
                        game_util:playSection(anim_label_name_cfg.qianjin);
                    else
                        ...
                        game_util:playSection(anim_equence[animIndex][1]);
                        animIndex = animIndex + 1;
                    end
                end
            end
            ...
            local function moveEndCallFunc()
                ...
                game_util:playSection(self.m_attack_object.animNode , anim_equence[animIndex][1]);
                animIndex = animIndex + 1;
            end
            ...
            elseif attack_type == "jingong" then
                ...
                game_util:playSection(self.m_attack_object.animNode , anim_label_name_cfg.qianjin);
            end
        end
    ...
    local function runCallFunc()
        ...
        local function runRealAction()
           ...
           -- elseif (self.m_skill_type == "skill_pet") then
           --     runPetSkillOtherSequence(self,t_params,nil,other_sequence)
           else
               self.m_attack_object.animNode:registerScriptAnchor(t_params.anchorCallFunc);
...
--伤害动画特效
function game_battle_skill_pets.createHurtAnim(self,t_params)
    ...
    local function onAnimSectionEnd(animNode, theId,theLabelName)
        ...
        if animIndex > animCount then
            ...
            game_util:playSection(mAnimNode , hurtAnim_equence[animIndex][2]);
            animIndex = animIndex + 1;
        end
    end
    ...
    -- mAnimNode:playSection(hurtAnim_equence[animIndex][1]);
    mAnimNode:setRhythm(hurtAnim_equence[animIndex][2]);
    game_util:playSection(mAnimNode , hurtAnim_equence[animIndex][1]);
    animIndex = animIndex + 1;
...
function game_battle_skill_pets.getStartAndEndPos(self,t_params)
...
--召唤动画特效
function game_battle_skill_pets.createCallAnim(self,t_params)
    ...
    local function delayCallFunc()
        ...
        local function onAnimSectionEnd(animNode, theId,theLabelName)
            if animIndex > animCount then
                ...
                if theLabelName == anim_label_name_cfg.qianjin then
                    ...
                    game_util:playSection(animNode , theLabelName);
                else
                    -- animNode:playSection(callAnim_equence[animIndex][1])
                    animNode:setRhythm(callAnim_equence[animIndex][2]);
                    game_util:playSection(animNode , callAnim_equence[animIndex][1]);
                    animIndex = animIndex + 1;
                end
            end
        end
        ...
        local function moveEndCallFunc()
            ...
            mAnimNode:registerScriptTapHandler(onAnimSectionEnd);
            mAnimNode:setRhythm(callAnim_equence[animIndex][2]);
            game_util:playSection(mAnimNode , callAnim_equence[animIndex][1]);
            animIndex = animIndex + 1;
        end
        if attack_type == "yuangong" then
            moveEndCallFunc();
        elseif attack_type == "jingong" then
            ...
            -- mAnimNode:playSection(anim_label_name_cfg.qianjin);
            game_util:playSection(mAnimNode , anim_label_name_cfg.qianjin);
        end
...
--显示一次，但效果确持续
function game_battle_skill_pets.addBuffOnceCircel(self,t_params)
   ...
   cclog("game_battle_skill_pets.addBuffOnceCircel")
...
function game_battle_skill_pets.addBuffOnce(self,t_params)
    cclog("function:------------------------addBuffOnce");
   t_params.enemy_object_tab = self.m_attack_object;
   t_params.other_sequence = t_params.skills_table.other_sequence;
   cclog("game_battle_skill_pets.addBuffOnce")
   local other_sequence = t_params.skills_table.other_sequence;
   if(t_params.skills_table.aniState ~= nil) then
       self.m_attack_object.aniState = t_params.skills_table.aniState
   end
...
function game_battle_skill_pets.addBuffCircel(self,t_params)
    cclog("function:------------------------addBuffCircel");
   t_params.enemy_object_tab = self.m_attack_object;
   t_params.other_sequence = t_params.skills_table.other_sequence;
   cclog("game_battle_skill_pets.addBuffCircel")
   local other_sequence = t_params.skills_table.other_sequence;
...
--创建buff动画
function game_battle_skill_pets.createBuffAnim(self,t_params)
    ...
    local function delayCallFunc()
      ...
      local function onAnimSectionEnd(animNode, theId,theLabelName)
        ...
        game_util:playSection(animNode , buffAnim_equence[animIndex][1]);
        animIndex = animIndex + 1;
      end
...
function game_battle_skill_pets.createNormalAttack(self,t_params)
...
function game_battle_skill_pets.createGroupAttack(self,t_params)
...
--创建近身的顺序攻击
function game_battle_skill_pets.createOrderOfAttack(self,t_params)
...
function game_battle_skill_pets.create(self,t_params)
    ...
    -- cclog2(skillData,"skillData==2==2=2=2=2==")
    ...
    elseif t_params.skill_type == "skill_pet" then

    elseif t_params.skill_type == "pet_add_buff" then--宠物放buff
    ...
    --攻击者不存在直接返回
    if (not (t_params.skill_type == "skill_hero" or t_params.skill_type == "skill_pet" or t_params.skill_type == "skill_combiner") and self.m_attack_object == nil) then
    ...
    elseif t_params.skill_type == "skill_pet" then--宠物技能
        local petAnim = ""
        local initData = self.params.game_battle_table.m_battleInitData
        local camp = self.params.camp
        if camp == 1 then
            local pet_id_att = initData.pet_id_att
            if initData.pet_att then
                local pet_att = initData.pet_att[pet_id_att]
                petAnim = pet_att.animation
            end
        elseif camp == -1 then
            local pet_id_def = initData.pet_id_def
            if initData.pet_def then
                local pet_def = initData.pet_def[pet_id_def]
                petAnim = pet_def.animation
            end
        end
        local mAnimNode = game_util:createIdelAnim(petAnim);
        if mAnimNode == nil then
            self.params.game_battle_table:setCanGo(true,"no pet anim file" .. tostring(petAnim));
            self:destroy();
            return;
        end
        mAnimNode:setPosition(self.m_attackCenterPosTable[5])
        mAnimNode:setAnchorPoint(ccp(0.5,0))
        mAnimNode:setFlipX(camp ~= 1)
        mAnimNode:setRhythm(public_config.action_rythm)
        self.m_attack_object = {aniState = 0,camp = camp}
        self.m_attack_object.animNode = mAnimNode
        self.m_battle_layer:addChild(mAnimNode);
        -- skillData.name = "leader_phsc_bash"
        -- skill_name = "leader_skill." .. skillData.name;
        -- skillData.name = petAnim .. "_s1"
        skill_name = petAnim .. "." .. skillData.name;
        fileFullPath = CCFileUtils:sharedFileUtils():fullPathForFilename("skill_effect/" .. petAnim .. "/" .. skillData.name ..".lua");
    elseif t_params.skill_type == "skill_combiner" then--神兽技能
        local combinerAnim = ""
        local initData = self.params.game_battle_table.m_battleInitData
        local camp = self.params.camp
        local beast_num = skillData.beast_num
        if camp == 1 then
            local beast_id_att = beast_num
            -- cclog2(beast_id_att,"beast_id_attbeast_id_att")
            if initData.beast_att then
              -- cclog2(initData.beast_att,"initData.beast_attinitData.beast_attinitData.beast_att")
              local beast_att = nil;
              for k,v in pairs(initData.beast_att) do
                if v.beast_num == beast_id_att then
                  beast_att = initData.beast_att[k]
                end
              end
                
                -- cclog2(beast_att,"beast_att=======")
                beastAnim = beast_att.animation
            end
        elseif camp == -1 then
            local beast_id_def = beast_num
            -- cclog2(beast_id_att,"beast_id_attbeast_id_att")
            if initData.beast_def then
              -- cclog2(initData.beast_att,"initData.beast_attinitData.beast_attinitData.beast_att")
              local beast_def = nil;
              for k,v in pairs(initData.beast_def) do
                if v.beast_num == beast_id_def then
                  beast_def = initData.beast_def[k]
                end
              end
                -- cclog2(beast_att,"beast_att=======")
                beastAnim = beast_def.animation
            end
        end
        local mAnimNode = game_util:createIdelAnim(beastAnim);
        if mAnimNode == nil then
            self.params.game_battle_table:setCanGo(true,"no beast anim file" .. tostring(beastAnim));
            self:destroy();
            return;
        end
        mAnimNode:setPosition(self.m_attackCenterPosTable[5])
        ...
        -- skillData.name = "leader_phsc_bash"
        -- skill_name = "leader_skill." .. skillData.name;
        -- skillData.name = beastAnim .. "_s1"
        -- cclog2(skillData,"skillDataskillData")
        skill_name = beastAnim .. "." .. skillData.name;
        fileFullPath = CCFileUtils:sharedFileUtils():fullPathForFilename("skill_effect/" .. beastAnim .. "/" .. skillData.name ..".lua");
    elseif t_params.skill_type == "buff_skill" then
    ...
    elseif t_params.skill_type == "skill_master" then--主角技能

        skill_name = self.m_attack_object.animFile .. "." .. skillData.name;
        local skillFile_name = self.m_attack_object.animFile .. "/" .. skillData.name
        if string.find(skill_name,"zhujue") or string.find(skill_name,"zhujiao") then
           skillFile_name = "zhujiao_nan/" .. skillData.name
           skill_name = "zhujiao_nan." .. skillData.name
         end
        fileFullPath = CCFileUtils:sharedFileUtils():fullPathForFilename("skill_effect/" .. skillFile_name ..".lua");
    else
        skill_name = self.m_attack_object.animFile .. "_battle." .. skillData.name;
        
        local skillFile_name = self.m_attack_object.animFile .. "_battle/" .. skillData.name
    ...
    local skill_detail_cfg = getConfig("pet_skill_detail");
    if self.params.skill_type == "skill_master" then
        skill_detail_cfg = getConfig("main_hero_skill");
    end
    ...
    elseif t_params.skill_type == "skill_pet" then
       self.m_camp = t_params.camp
       self.m_skillid = skillData.skillid
       cclog("skill_pet---------skillData.skillid---------->"..tostring(skillData.skillid))
       local skills_table = require ("skill_effect.".. self.m_skillName);
        local function animEnd(node,theId,lableName)
            self.m_attack_object.animNode:registerScriptTapHandler(self.params.game_battle_table.animEnd);
            skills_table:skillCallFunc(self,self.params.game_battle_table,self.params.currentFrameData);
        end
        self.m_attack_object.animNode:registerScriptTapHandler(animEnd);
        -- self.m_attack_object.animNode:playSection(anim_label_name_cfg.dengchang);
        game_util:playSection(self.m_attack_object.animNode , anim_label_name_cfg.dengchang);
    elseif t_params.skill_type == "skill_combiner" then
       self.m_camp = t_params.camp
       self.m_skillid = skillData.skillid
       cclog("skill_combiner---------skillData.skillid---------->"..tostring(skillData.skillid))
       local skills_table = require ("skill_effect.".. self.m_skillName);
        local function animEnd(node,theId,lableName)
            self.m_attack_object.animNode:registerScriptTapHandler(self.params.game_battle_table.animEnd);
            skills_table:skillCallFunc(self,self.params.game_battle_table,self.params.currentFrameData);
    ...
    elseif skillData.skillid then
      if t_params.skill_type == "skill_master" then
        game_util:createSkillNameTips(self.m_attack_object.animNode,skillData.skillid,nameOverStartSkill,nil,"skill_master");
      else
        game_util:createSkillNameTips(self.m_attack_object.animNode,skillData.skillid,nameOverStartSkill,nil,"pets");
      end
...
function game_battle_skill_pets.print(self)
	cclog("self.params ===" .. tostring(self.params));
end
--[[--
    创建新表
]]
function game_battle_skill_pets.new(self)
	  local tab = {};
...
--移动动画特效
function game_battle_skill_pets.createMoveAnim(self,t_params)
...
--创建特效 
function game_battle_skill_pets.createEffectsAnim(self,t_params)
